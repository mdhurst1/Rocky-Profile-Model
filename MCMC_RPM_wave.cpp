...
void MCMC_RPM::RunMetropolisChain(int NIterations, char* ParameterFilename, char* OutFilename)
{
	...
	double  Resistance_New, Resistance_Old, Resistance_Min, Resistance_Max, Resistance_Std, Resistance_Init,
            ...
            WaveDecay_New, WaveDecay_Old, WaveDecay_Min, WaveDecay_Max, WaveDecay_Std, WaveDecay_Init,
            SubmarineDecayConst;
    ...
    double TidalRange;
    double dFR, dK, dY; //change in parameter values for Resistance (FR), WeatheringRate (K) and Wave height decay rate (Y)
	...
	ChainFileOut  << "i Resistance_New WeatheringRate_New WaveDecay_New NewLikelihood LastLikelihood NAccepted NRejected" << endl;
    ...
    ParamFileIn >> Dummy >> Resistance_Min >> Dummy >> Resistance_Max >> Dummy >> Resistance_Std >> Dummy >> Resistance_Init
                ...
                >> Dummy >> WaveDecay_Min >> Dummy >> WaveDecay_Max >> Dummy >> WaveDecay_Std >> Dummy >> WaveDecay_Init
	            >> Dummy >> TidalRange >> Dummy >> dZ >> Dummy >> dX >> Dummy >> Gradient >> Dummy >> CliffHeight >> Dummy >> CliffFailureDepth
                >> Dummy >> MinElevation >> Dummy >> SubmarineDecayConst >> Dummy >> RSLFilename >> Dummy >> StartTime >> Dummy >> ZStd;
	...
	//double WaveAttenuationConst = 0.01; - this is now a free parameter (WaveDecay_Init)
	MCMCPlatform.Set_WaveCoefficients(StandingCoefficient, BreakingCoefficient, BrokenCoefficient, WaveDecay_Init);
	...
	for (int j=0; j<NIterations; ++j)
	{
	  ...
	  //First Update Resistance
        ...
        {	
			Rand1 = (double)rand()/RAND_MAX; Rand2 = (double)rand()/RAND_MAX;
	  		dY = MeanChange + WaveDecay_Std*sqrt(-2.*log(Rand1))*cos(2.*M_PI*(Rand2));
	  		WaveDecay_New = WaveDecay_Old + dY;
			if ((WaveDecay_New < WaveDecay_Min) || (WaveDecay_New > WaveDecay_Max)) continue;
			else Accept = 1;
	  	} 
	    ...
	    double BreakingCoefficient = 10.;
	    double BrokenCoefficient = 1.;
	    //double WaveAttenuationConst = 0.01; now free paramater (WaveDecay_New)
	    MCMCPlatform.Set_WaveCoefficients(StandingCoefficient, BreakingCoefficient, BrokenCoefficient, WaveDecay_New);
		...
		ChainFileOut  << j << " " 
		              << Resistance_New << " " << WeatheringRate_New << " " << WaveDecay_New << " "
