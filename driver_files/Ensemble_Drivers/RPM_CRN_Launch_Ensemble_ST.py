...
#Cliff failure depth
SubtidalEfficacy = []
SubtidalEfficacy.append(0.0005)
SubtidalEfficacy.append(0.005)
SubtidalEfficacy.append(0.05)
SubtidalEfficacy.append(0.5)
SubtidalEfficacy.append(0.8)
#SubtidalEfficacy.append(50.0)

#print(len(CliffFailureDepth))
...
#for(int p=0, Np = WaveAttenuationConst.size(); p<Np; p+=2)
for i in range(0, len(SubtidalEfficacy)):
	Launchstr = Folder+"Ensemble_Drivers/RPM_CRN_Ensemble.out "+ Folder+" "+"Ensemble "+ str(CRNFlag) +" "+ str(Gradient) +" "+ str(TidalRanges) +" "+ str(SubtidalEfficacy[i]) +" "+ str(WaveHeight) +" "+ str(Resistances) +" "+ str(WaveAttenuationConst) +" "+ str(WeatheringRates) +" "+ str(CliffFailureDepth)
	#Track Run
	Run +=1
