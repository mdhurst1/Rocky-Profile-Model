...
#WaveHeight = 3
WaveAttenuationConst = -1.15
Resistances = 1.24
WeatheringRates = -2.55
...
#for(int p=0, Np = WaveAttenuationConst.size(); p<Np; p+=2)
for i in range(0, len(WaveHeight)):
	Launchstr = Folder+"Ensemble_Drivers/RPM_CRN_Ensemble.out "+ Folder+" "+"Ensemble "+ str(CRNFlag) +" "+ str(Gradient) +" "+ str(TidalRanges) +" "+ str(SubtidalEfficacy) +" "+ str(WaveHeight[i]) +" "+ str(Resistances) +" "+ str(WaveAttenuationConst) +" "+ str(WeatheringRates) +" "+ str(CliffFailureDepth)
	#Track Run
	Run +=1
