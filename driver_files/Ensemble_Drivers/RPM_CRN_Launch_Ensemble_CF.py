...
#Cliff failure depth
CliffFailureDepth = []
CliffFailureDepth.append(0.1)
CliffFailureDepth.append(0.5)
CliffFailureDepth.append(1.0)
CliffFailureDepth.append(5.0)
CliffFailureDepth.append(10.0)
CliffFailureDepth.append(50.0)
...
#for(int p=0, Np = WaveAttenuationConst.size(); p<Np; p+=2)
for i in range(0, len(CliffFailureDepth)):
	Launchstr = Folder+"Ensemble_Drivers/RPM_CRN_Ensemble.out "+ Folder+" "+"Ensemble "+ str(CRNFlag) +" "+ str(Gradient) +" "+ str(TidalRanges) +" "+ str(SubtidalEfficacy) +" "+ str(WaveHeight) +" "+ str(Resistances) +" "+ str(WaveAttenuationConst) +" "+ str(WeatheringRates) +" "+ str(CliffFailureDepth[i])
	#Track Run
	Run +=1
